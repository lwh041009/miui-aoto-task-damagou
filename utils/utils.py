...
def get_token_by_captcha(url: str) -> Union[str, bool]:
    ...
    try:
        ...
        captcha_id = query_params.get("captcha_id") or query_params.get("c", "") or query_params.get("k", "")
        e_param = query_params.get("e", "")

        # 小米验证码URL参数说明:
        # c=captcha_id(当gt用), l=challenge(极验3代参数)
        challenge = query_params.get("l", "") or query_params.get("challenge", "")

        if not challenge:
            log.error("验证码URL中未找到challenge参数")
            return False

        log.info(f"获取到极验参数: gt={captcha_id[:16]}..., challenge={challenge[:16]}...")

        # 调用打码狗v3解算（极验3代）
        geetest_data = get_validate(captcha_id, challenge)

        if not geetest_data.validate:
            log.error("打码狗解码失败")
            return False

        log.success("打码狗v3解码成功")

        # 用解码结果提交验证（小米验证接口对复杂UA/headers敏感，使用简化headers）
        verify_headers = {
            "Accept": "*/*",
            "Content-type": "application/x-www-form-urlencoded",
            "Origin": "https://web.vip.miui.com",
        ...
        log.debug(f"verify response: {response.text}")
