...
DEFAULT_QR_USER_AGENT = (
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
    "(KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36 Edg/120.0.0.0"
)
DEFAULT_QR_LOGIN_URL = (
...
def _login_qs() -> str:
    sid = _env("MIUITASK_LOGIN_SID", "miui_vip_a")
    locale = _env("MIUITASK_LOGIN_LOCALE", "zh_CN")
    query = urlencode({"_json": "true", "sid": sid, "_locale": locale})
    return quote(f"?{query}", safe="")
...
class Login:
    ...
    def __init__(self, account: Account) -> None:
        ...
        self.device_id = _env(
            "MIUITASK_DEVICE_ID",
            account.deviceId or self.cookies.get("deviceId", "S13aukyf5y2jecCG"),
        )
        self.pass_ua = _env(
            "MIUITASK_PASS_UA",
            account.pass_ua or self.cookies.get("pass_ua", "web"),
        )
        self.u_locale = _env(
            "MIUITASK_U_LOCALE",
            account.uLocale or self.cookies.get("uLocale", "zh_CN"),
        )

    def _login_cookies(self) -> Dict[str, str]:
        return {
            "deviceId": self.device_id,
            "pass_ua": self.pass_ua,
            "uLocale": self.u_locale,
        }
    ...
    def login(
        ...
        data = {
            "qs": _login_qs(),
            "callback": _env("MIUITASK_LOGIN_CALLBACK", "https://api-alpha.vip.miui.com/sts"),
            "_json": "true",
            "_sign": _env("MIUITASK_LOGIN_SIGN", "eQzFP7RKdHfN0VKBbp86ZVzlgq0="),
            "user": self.uid,
            "hash": self.password,
            "sid": _env("MIUITASK_LOGIN_SID", "miui_vip_a"),
            "_locale": _env("MIUITASK_LOGIN_LOCALE", "zh_CN"),
        }
        try:
            if (
                self.cookies != {}
    ...
    def get_cookies_by_passtk(self, user_id: str, pass_token: str):
        ...
        try:
            ...
            params = {
                "destUrl": _env(
                    "MIUITASK_LOGIN_DEST_URL",
                    "https://web.vip.miui.com/page/info/mio/mio/checkIn?app_version=dev.230904",
                ),
    ...
    def qr_login(self) -> Tuple[str, bytes]:
        ...
        headers = {
            ...
            "User-Agent": _env(
                "MIUITASK_QR_USER_AGENT",
                self.user_agent or DEFAULT_QR_USER_AGENT,
            ),
        ...
        response = get(
            _env("MIUITASK_QR_LOGIN_URL", DEFAULT_QR_LOGIN_URL),
            headers=headers,
        )
